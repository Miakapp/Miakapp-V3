...
import { requireSameOriginAuthDomain } from './configured-host';

describe('configured staging host', () => {
  it('keeps Firebase redirect auth on the same Hosting origin', () => {
    expect(requireSameOriginAuthDomain('home.example.test', {
  ...
  it('rejects cross-origin redirect auth on HTTPS', () => {
    expect(() => requireSameOriginAuthDomain('project.firebaseapp.com', {
