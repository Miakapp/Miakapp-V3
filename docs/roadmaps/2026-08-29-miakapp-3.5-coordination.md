...
complete Section 18 emulator or staging gates.
...
7. emulator-first tests for rules and Functions — the Section 5–7 slice is
   complete; push, publication and remaining admission evidence are open.
...
7. **Done 2026-09-01** — implement the first owner-to-access-token vertical
   ...
   verification.
8. Extend the emulator implementation through push consent, component
   publication/read-back, bounded audit/rate/cost admission and the remaining
   RFC 0004 Section 18 fault matrix before any staging deployment.
...
conformance, the real Node-RED adapter, production push grants and publisher
authorization, the React adapter and Firebase artifact delivery remains
**medium** until their vertical slices exercise the accepted contracts end to
end. Confidence in the now-executed owner/Home-Key/access-token emulator path is
**high within the documented emulator boundary**, but it is not production or
staging evidence.
