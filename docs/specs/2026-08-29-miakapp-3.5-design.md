...
3. **Platform control plane — closed 2026-08-31** — RFC 0004 defines owner
   ...
   a recording FCM transport. The Local Emulator Suite provides neither an App
   Check nor an FCM service emulator, so real App Check enforcement, FCM
   acceptance/delivery, publication, remaining admission evidence, complete
   Section 18 and staging behavior remain implementation exit gates.
...
semantic sends, while publication authority, real push-service integration,
remaining admission controls, session machines, queueing and disconnect behavior
still have to pass their vertical-slice or staging tests. Contract-kit, synthetic
service seams or partial-emulator self-conformance alone is not production
readiness.
