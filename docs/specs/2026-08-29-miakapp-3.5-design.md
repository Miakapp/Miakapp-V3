...
| `users/{uid}/pushDestinations/{destinationID}` | challenge-proved private push delivery addresses | Functions/IAM only; client rules deny all |
...
3. **Platform control plane — closed 2026-08-31** — RFC 0004 defines owner
   ...
   its initial Rules boundary. Push, publication, complete Section 18 and staging
   behavior remain implementation exit gates.
...
`control-plane/`; the remaining session machines, push/publication authority,
rules, Functions, queueing and disconnect behavior still have to pass their
vertical-slice tests. Contract-kit or partial-emulator self-conformance alone is
