...
6. **Trusted browser relay client — audience-bound local path closed 2026-09-04;
   ...
   acceptance execution. Plan revision 11 preserves revision 10 byte for byte,
   pins that result and closes the monitoring precondition. The separate
   ...
   Its separate preflight accepted only exact merged commit
   ...
   fixed staging origin through an ephemeral loopback-only TLS listener plus an
   exact host-resolver mapping; a catch-all rule makes every unmatched hostname
   fail resolution. The certificate is generated by the reviewed absolute OpenSSL
   path and its key files are erased before any page request. Firefox and WebKit use
   exact-origin Playwright fulfilment. A strict offline content-security policy
   applies to every page, and each page must prove that one fixed external request
   is blocked. The deterministic artifact carries 204 exact repository
   dependency/validation files, a 13-file `playwright-core`
   runtime/licence/type-contract subset and one generated module allowlist. Required
   validation assets—including the exact relay-service Terraform and edge README bytes
   re-hashed by transitive validators—remain inert data and only 87 reviewed runtime
   modules are importable; browser binaries, unreferenced package files, unrelated
   documentation and the controlled owner testing entrypoint are excluded. A dedicated
   credential-free workflow preserves two real
   240-second renewal boundaries and proves the production page runtime across
