...
| `LOCAL-20` | An unauthenticated poster files a `miakapp.runtime-diagnostics/1` report carrying a code the trusted host never raises, a prototype-chain name, a release with whitespace or newlines, a foreign schema, extra keys, a non-UTC or backdated instant, or reaches a sink whose writer throws | The report is rejected with `invalid_request` rather than recorded; the observation timestamp is the server's; the accepted line carries only the four checked fields; a failing sink never turns a diagnostic into a request failure | `test/unit/runtime-diagnostics.test.ts` — ingest vocabulary and sink cases |
