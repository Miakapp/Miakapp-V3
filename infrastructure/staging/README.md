...
| [`browser-relay-page/`](browser-relay-page/) | Dormant Firebase/MiakAPI scenario host and deterministic artifact | Revision 3 implements serialized lifecycle and typed outcomes; pinned browsers prove native terminal fencing and explicit cleanup using offline fakes, while the separate native Chromium driver proves persisted BFCache restoration; no live authority is granted |
...
| [`browser-relay-source-transports/`](browser-relay-source-transports/) | Dormant seven-source transport protocol | Pulls the exact 22 stages and 43 observations from trusted source-specific readers with required EOF, downstream backpressure, operation/stage identity binding, bounded drain, exact close and permanent poison; adjacent dormant capability readers and concrete session adapters are present, while session producers, network, credentials, case-adapter wiring, process IPC and authority remain absent |
...
BFCache testing, so the legacy bridge retains state
`blocked_by_pinned_playwright`; its simulated trusted persisted unit test is
not native BFCache proof. A dedicated pinned-CDP driver now supplies that proof
offline without weakening the high-level bridge's blocker.
...
blocker was closed by the revision-99 semantic object assembled from a small
...
96-KiB ceiling. That baseline bundle was 194,718 bytes, leaving 67,426 bytes
below its reviewed 256-KiB aggregate ceiling. That representation-only revision
...
202,086 bytes. The current revision-101 bundle additionally records the
concrete dormant source-session adapters and totals 211,303 bytes, leaving
50,841 bytes below the same six-fragment 262,144-byte ceiling.
The adjacent
...
instances and two memory-only private inputs, executes all 11 controller phases,
derives all observations internally and closes the existing 18-fact receipt
only after both pages terminate. Pinned Chromium navigates away with Playwright,
restores through `Page.navigateToHistoryEntry` and reads the restored page
through `Runtime.evaluate`. Success requires the actual Chromium sequence: a
trusted persisted `pagehide` dispatched while visible, a trusted transition to
hidden after suspension, and a trusted persisted `pageshow` dispatched and
completed while visible. That page witness must agree with exactly one main-frame
`BackForwardCacheRestore`, with no `Page.backForwardCacheNotUsed` event. Its
smoke uses the reviewed Hosting `no-store` and security headers; current or
latched diagnostics, active trace/HAR/video/logger capture, suppressed
cancellation, pending sensitive protocol transitions, capture attempts during
the scenario, late browser work and failed cleanup all fail the run. This is a
trusted, exclusively owned Playwright-process harness rather than a same-realm
sandbox: all three injected providers, page navigation/content/init scripts and
...
The Chromium layer composes its 18 page facts and delegates the two secondary
page paths. The adjacent
[`browser-relay-secondary-case-adapter/`](browser-relay-secondary-case-adapter/)
now fills those paths with the unchanged Playwright bridge: each complete
Firefox/WebKit fact is validated, reduced to the five reviewed projection
fields and recorded into the active `LIVE-10` scope, while the evidence session
remains the sole authoritative envelope, timing, receipt and result owner. Its
real offline smoke closes both pages and the complete 40-assertion schedule.
The bridge receipts prove page closure only and are discarded. The composition
also drains bridge-surviving page/private-input acquisitions and closes any page
that arrives after cancellation before downstream teardown. It does not own
fixture creation/removal. The adjacent
[`browser-relay-independent-case-adapter/`](browser-relay-independent-case-adapter/)
now fills its final downstream slot. Seven fixed source observers receive only
revocable `{ browser, case_id, signal, record }` capabilities; the adapter
derives source kind and sequence, validates each complete observation and
projects only `{ observation }` while the evidence session remains the sole
...
methods for its canonical fact kinds; the package binds every stage to one
opaque operation capability with a bounded absolute expiry, invokes kinds
lazily and in order, and semantically validates each sanitized observation
before returning it. It accepts no token, key or other credential material,
performs no source or network I/O, and clears its authority references on
terminal closure. The adjacent
[`browser-relay-source-authority-adapters/`](browser-relay-source-authority-adapters/)
package now supplies the concrete dormant bridge from seven exact fixed-scope
ephemeral sessions to those kind-specific authorities. It binds the opaque
operation capability on first use, independently revalidates all 32 methods in
canonical order, bounds reads and the public-close wrapper, sanitizes
cancellation and releases each session only after its started callbacks settle.
This remains a trusted same-process contract: source-session producers,
transport-to-case wiring and validated IPC remain open before any live
execution.
The adjacent
...
and their dormant authenticated-capability reader/provider protocol are
implemented separately. Concrete ephemeral-session authority adapters now
close the next boundary, but source-session producers, case-adapter wiring and
trusted live page providers remain absent. These offline
capabilities are not acceptance evidence. The adjacent
...
and dormant authenticated-capability reader/provider protocols and concrete
fixed-scope ephemeral-session adapters are present; source-session producers,
case-adapter wiring, trusted live page providers and dedicated-process
