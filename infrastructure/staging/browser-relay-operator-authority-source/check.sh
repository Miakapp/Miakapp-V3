...
for source in contract.mjs guard.mjs internal.mjs source.mjs testing.mjs
do
  node --check "${package_root}/${source}"
done

node "${package_root}/guard.mjs" "${package_root}"
node --test "${repository_root}/infrastructure/staging/test/browser-relay-operator-authority-source.test.mjs"
