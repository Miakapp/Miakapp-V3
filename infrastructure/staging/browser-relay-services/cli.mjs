...
export const relayServicesRoot = dirname(fileURLToPath(import.meta.url));
...
export function run(command, args, options = {}) {
  const result = spawnSync(command, args, {
    cwd: options.cwd ?? relayServicesRoot,
    env: options.env ?? childEnvironment(),
    maxBuffer: options.maxBuffer ?? 64 * 1024 * 1024,
    timeout: options.timeout ?? 20 * 60 * 1_000,
...
export function parseJson(bytes, description, maximumBytes = 64 * 1024 * 1024) {
...
export function terraformEnvironment(terraformData, accessToken) {
  ...
  return childEnvironment(process.env, {
    ...
    TF_CLI_CONFIG_FILE: join(relayServicesRoot, 'terraform-cli.tfrc'),
