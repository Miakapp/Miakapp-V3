...
export function validateGitHubPolicy(value, options = {}) {
  ...
  exact(activation.cloud_plan_authorized, false, 'activation.cloud_plan_authorized');
...
export function verifyRetiredWorkflow(repositoryRoot, policy) {
  const root = resolve(repositoryRoot);
  const resolveInsideRepository = (label, relativePath) => {
  ...
  const activePath = resolveInsideRepository(
    'retired workflow',
    policy.activation.active_workflow_path,
  );
  if (lstatSync(activePath, { throwIfNoEntry: false }) !== undefined) {
    reject('retired workflow', 'must be absent');
  }

  const blueprintPath = resolveInsideRepository(
    'historical workflow blueprint',
    policy.activation.blueprint_path,
  );
  const blueprintMetadata = lstatSync(blueprintPath, { throwIfNoEntry: false });
  if (
    blueprintMetadata === undefined
    || !blueprintMetadata.isFile()
    || blueprintMetadata.isSymbolicLink()
  ) {
    reject('historical workflow blueprint', 'must be a regular non-symlink file');
  }
  if (blueprintMetadata.size > MAX_WORKFLOW_BYTES) {
    reject('historical workflow blueprint', `must be at most ${MAX_WORKFLOW_BYTES} bytes`);
  }
  const digest = createHash('sha256').update(readFileSync(blueprintPath)).digest('hex');
  exact(digest, policy.activation.workflow_sha256, 'historical workflow blueprint SHA-256');
  return digest;
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (path === undefined || !supportedMode || args.length !== (mode === undefined ? 1 : 2)) {
    ...
    try {
      ...
      verifyRetiredWorkflow(resolve(dirname(path), '../../..'), policy);
      console.log(`Validated ${policy.schema}; manual keyless partial-foundation recovery is retired.`);
    } catch (error) {
