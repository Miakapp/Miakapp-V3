...
import { readAndValidateInitialWorkloadPlan } from './validate-plan.mjs';

const APPLY_AUTHORIZATION = 'MIAKAPP_STAGING_WORKLOAD_APPLY_AUTHORIZATION';
...
function privateBundle(path) {
  ...
  if (!entry.isDirectory()
    ...
    throw new Error('Workload apply requires an exact private bundle directory');
  }
  return bundle;
}
...
function verifyVariables(path, metadata, bundle) {
  ...
  try {
    ...
    throw new Error('Private workload variables are invalid JSON');
  }
  if (canonicalJson(variables) !== bytes.toString('utf8')
    ...
    throw new Error('Private workload variables no longer match the reviewed plan');
  }
  return variables;
}
...
async function main() {
  ...
  const { value: metadata } = readPlanMetadata(join(bundle, 'metadata.json'));
  ...
  if (sha256(planBytes) !== metadata.terraform_plan_sha256
    ...
    throw new Error('Private workload bundle digest verification failed');
  }
  ...
  readAndValidateInitialWorkloadPlan(planJsonPath, {
  ...
  try {
    ...
    if (convergence.status !== 0) {
      if (!existsSync(join(bundle, 'convergence-failure.log'))) {
        const diagnostics = Buffer.concat([
      ...
      throw new Error(applyFailed
        ? 'Workload apply failed and live state is incomplete; private diagnostics were preserved'
        : 'Workload apply completed but the follow-up plan is not empty');
    }
    ...
    process.stdout.write([
      applyFailed
        ? 'The provider returned an error after the exact workload converged; live state was reconciled.'
        : 'The exact private staging workload plan was applied and converged.',
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : 'Private workload apply failed');
  process.exitCode = 1;
});
