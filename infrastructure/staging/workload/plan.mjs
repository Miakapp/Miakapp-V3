...
const PLAN_CONFIRMATION = 'MIAKAPP_STAGING_WORKLOAD_PLAN_CONFIRMATION';
...
async function main() {
  ...
  if (process.env[PLAN_CONFIRMATION] !== PROJECT_ID) {
    throw new Error(`Set ${PLAN_CONFIRMATION}=${PROJECT_ID} to acknowledge the exact private deployment target`);
  }
  ...
  try {
    ...
    if (plan.status !== 2) throw new Error('Initial private workload plan must contain a reviewed create-only delta');
    ...
    const summary = readAndValidateInitialWorkloadPlan(planJsonPath, {
      repositoryCommit,
    ...
    process.stdout.write([
      `Private workload bundle: ${bundle}`,
      `Plan SHA-256: ${metadata.terraform_plan_sha256}`,
      `Source SHA-256: ${metadata.source_archive_sha256}`,
      `Authorization: ${workloadAuthorization(planBytes, repositoryCommit)}`,
      `Planned delta: ${summary.create} creates, 0 updates, 0 deletes; internal-only, scale 0..1, no live request.`,
      '',
    ].join('\n'));
  } finally {
...
main().catch((error) => {
  console.error(error instanceof Error ? error.message : 'Private workload planning failed');
  process.exitCode = 1;
});
