...
Status: reviewed one-shot implementation; not yet planned or executed
...
The planner requires the exact registered reCAPTCHA Enterprise provider, zero
enforcement records, zero debug tokens, one active Firebase Web app, and an
empty default Hosting site with zero versions and releases. It builds two
...
2. create one atomic, non-deleted GCS operation claim;
3. create and release one labelled Hosting version on only
   `/__acceptance/app-check/index.html`;
4. launch one headed Playwright Chromium context and call the production
   reCAPTCHA Enterprise provider exactly once;
5. return only token shape, TTL and duration counters from the page, never the
   token; and
...
The planner prints an exact authorization bound to the canonical metadata and
repository commit. Apply that bundle once:

```sh
MIAKAPP_STAGING_BROWSER_ATTESTATION_APPLY_AUTHORIZATION='run-browser-app-check-attestation:...' \
...
The planner prints a separate exact authorization. Its apply path creates at
...
inventory. It never deletes the default Hosting site, operation claim or App
Check provider.
