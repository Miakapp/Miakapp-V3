...
export const browserAppCheckRoot = dirname(fileURLToPath(import.meta.url));
...
export function run(command, args, options = {}) {
  const result = spawnSync(command, args, {
    cwd: options.cwd ?? browserAppCheckRoot,
...
export function parseJson(bytes, description, maximumBytes = 32 * 1024 * 1024) {
...
export function terraformEnvironment(terraformData, accessToken) {
  if (typeof accessToken !== 'string' || accessToken.length < 20
    || accessToken.length > MAXIMUM_TOKEN_BYTES || /\s/u.test(accessToken)) {
  ...
  return childEnvironment(process.env, {
    ...
    TF_CLI_CONFIG_FILE: join(browserAppCheckRoot, 'terraform-cli.tfrc'),
...
export function validateToolchain() {
  const value = runJson('terraform', ['version', '-json'], { description: 'terraform-version' });
...
export async function verifiedOperatorSession() {
  ...
  const principal = parseJson(bytes, 'Verified operator principal', 64 * 1024);
