...
allows three. Two real renewal intervals plus scenario overhead also require the
page's Chromium budget to grow from 480 to 600 seconds, and the current page API
does not yet drive the negative call and native lifecycle scenarios. Those
compatibility gaps and the absent Playwright bridge are explicit in
`profile.json`; the producer does not turn them into false acceptance evidence.
