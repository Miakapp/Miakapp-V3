...
export function validateBrowserRelayIndependentCaseAdapterRoot(rootUrl) {
  const root = lstatSync(rootUrl);
  if (!root.isDirectory() || root.isSymbolicLink()) {
    reject('Browser-relay independent case-adapter root must be one real directory');
  }
  ...
  if (JSON.stringify(entries.map(({ name }) => name).sort())
    ...
    reject('Browser-relay independent case-adapter inventory has drifted');
  }
  if (entries.some((entry) => !entry.isFile() || entry.isSymbolicLink())) {
    reject('Browser-relay independent case-adapter entries must be regular files');
  }
  ...
  for (const marker of [
    ...
    if (!sources['adapter.mjs'].includes(marker)) {
      reject('Independent case-adapter production entrypoint has drifted');
    }
  }
  for (const marker of [
    ...
    'stage.scope.record(source',
    'accepted !== true',
    'addAbortListener',
    'inFlightObserverTasks',
    'inFlightLifecycleTasks',
    'activeTransition',
    'browserStarted',
    'browserClosed',
    'cleanupConverged',
    'Promise.allSettled',
    'sourceObservers[source].close()',
    'browserLifecycle.close()',
    'Independent case adapter cannot own browser-page closure',
    'toJSON',
  ]) {
    if (!sources['internal.mjs'].includes(marker)) {
      reject('Independent case-adapter safety boundary has drifted');
    }
  }
  if (sources['adapter.mjs'].includes('ForTesting')
    ...
    reject('Independent case-adapter entrypoint separation has drifted');
  }
  const runtimeSource = [
    sources['adapter.mjs'],
  ...
  if (profile?.authority === null || typeof profile?.authority !== 'object'
    ...
    reject('Independent case-adapter profile exceeds its source-only authority');
  }
}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (process.argv.length !== 3 || rootPath === undefined) {
    console.error('Usage: node guard.mjs <browser-relay-independent-case-adapter-root>');
    process.exitCode = 2;
  } else {
