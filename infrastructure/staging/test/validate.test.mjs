...
test('accepts the successful and retired private user-relay probe', () => {
  ...
  assert.equal(validated.revision, 46);
  assert.equal(
    validated.status,
...
test('requires every remaining blocker and staging evidence row', () => {
  ...
  rejects((candidate) => {
    candidate.evidence.browser_app_check_prerequisite.global_attempt_claim.retry_authorized = true;
  }, /evidence\.browser_app_check_prerequisite\.global_attempt_claim\.retry_authorized/);
  rejects((candidate) => {
  ...
  rejects((candidate) => {
    candidate.evidence.browser_app_check_prerequisite.app_check_registered = true;
  }, /evidence\.browser_app_check_prerequisite\.app_check_registered/);
  rejects((candidate) => {
