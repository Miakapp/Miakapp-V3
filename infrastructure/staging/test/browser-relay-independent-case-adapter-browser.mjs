...
const documentSource = '<!doctype html><html><head><meta charset="utf-8">'
  + '<link rel="icon" href="data:,"><title>Offline independent case adapter</title></head>'
...
const clockController = Object.freeze({
...
async function chromiumScenario(dependencies, port, options) {
  ...
  const record = async (sequence) => {
    clockController.setAtLeastMilliseconds(
...
try {
  ...
  const schedulerRunner = (adapter, options) => runBrowserRelayCaseScheduleForTest(
    () => {
      ...
      browserStarts.chromium = clockController.lastMilliseconds();
      return session;
    },
    adapter,
    options,
  ...
  const result = await runBrowserRelayIndependentCaseScheduleForTesting(
    secondaryCaseRunner,
    {
      ...
      sourceObservers: independent.sourceObservers,
      browserLifecycle: independent.browserLifecycle,
    },
  );
  ...
  assert.equal(independent.observedCalls.length, 43);
  ...
  assert.equal(independent.browserLifecycleCloseCalls(), 1);
  assert.ok(Object.values(independent.observerCloseCalls).every((calls) => calls === 1));
  for (const browser of ['firefox', 'webkit']) {
    assert.equal(pages[browser].page.isClosed(), true);
  ...
  process.stdout.write(
    'firefox+webkit: real offline pages and 43 source observations closed one 40-assertion schedule.\n',
  );
} catch {
  await cleanupBrowsers();
  process.stderr.write(
    `Offline independent case-adapter browser smoke failed at ${stage}; raw diagnostics were discarded.\n`,
  );
  process.exitCode = 1;
}
